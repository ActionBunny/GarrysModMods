...
AccessorFunc( metaPly, "discpush", "DiscPush", FORCE_BOOL )
AccessorFunc( metaPly, "discthrower", "Discthrower" )
AccessorFunc( metaPly, "disc", "Disc" )
   ...
   for k, target in pairs(ents.FindInSphere(pos, radius)) do
      if IsValid(target) then
         ...
         if target:IsPlayer() and (not target:IsFrozen()) and ((not target.was_pushed) or target.was_pushed.t != CurTime()) then
			...
			target:SetDiscPush( true )
			target:SetDiscthrower( pusher )
			--target:SetDisc( placeholder )
...
function ENT:Explode(tr)
   if SERVER then
      ...
      self:SetSolid(SOLID_NONE)
		--placeholder = ents.Create( "ttt_discomb" )
...
ENT.Type   = "anim"
ENT.Base                = "base_anim"
ENT.PrintName           = "weapon_ttt_discomb"
...
scripted_ents.Register(ENT, "weapon_ttt_discomb", true)


local function DiscoGround( player, inWater, onFloater, speed )
	local tname = Format("DiscoPush_%d_%d", player:EntIndex(), math.ceil(CurTime()))
	timer.Create( tname, 1, 1, function() 
		player:SetDiscPush(false)
	end)
	
end

local function DiscoDamage( target, dmg )
	if target:IsPlayer() and target:Alive()  then
		if dmg:IsFallDamage() then
			--if IsValid( target:GetSuperPush() ) then
				if target:GetDiscPush() == true then
					local ent = ents.Create("weapon_ttt_discomb")
					dmg:SetAttacker( target:GetDiscthrower() )
...
hook.Add("EntityTakeDamage","DiscoDamage", DiscoDamage)
hook.Add("OnPlayerHitGround","DiscoGround", DiscoGround)