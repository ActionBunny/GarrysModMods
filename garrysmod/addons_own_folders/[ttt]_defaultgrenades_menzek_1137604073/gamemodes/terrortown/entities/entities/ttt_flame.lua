...
AccessorFunc(ENT, "fireorigin", "FireOrigin")
...
function StartFires(pos, tr, num, lifetime, explode, dmgowner, placeholder)
   for i=1, num do
      ...
      flame:Spawn()
      flame:PhysWake()
		if IsValid( placeholder ) then
			flame:SetFireOrigin( placeholder )
		end
...
function ENT:Explode()
   ...
   if IsValid( self:GetFireOrigin() ) then
	util.BlastDamage(self:GetFireOrigin(), dmgowner, pos, 300, 40)
	else
	util.BlastDamage(self, dmgowner, pos, 300, 40)
	end
end
...
function ENT:Think()
   ...
   if IsValid(self.firechild) then
      if self.next_hurt < CurTime() then
         ...
         if IsValid( self:GetFireOrigin() ) then
			dmg:SetInflictor( self:GetFireOrigin() )
         else
			dmg:SetInflictor(self.firechild)
		 end
