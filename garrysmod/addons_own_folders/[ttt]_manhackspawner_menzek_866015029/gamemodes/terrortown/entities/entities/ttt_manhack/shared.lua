-- c4 explosive
CreateConVar( "ttt_manhack_number", "10", FCVAR_NOTIFY + FCVAR_ARCHIVE , "Adjust the amount of Manhacks." )
...
AccessorFunc( metaEnts, "ismanhack", "IsManhack", FORCE_BOOL )
...
function ENT:Explode(tr)
   if SERVER then
      ...
      if tr.Fraction != 1.0 then
		...
		--placeholder = ents.Create( "ttt_weapon_manhack" )
	  ...
	  local num_manhacks = GetConVar( "ttt_manhack_number" ):GetFloat()

		for i=1, num_manhacks do
			local ent = ents.Create ("npc_manhack");
			ent:SetPos (pos + Vector(0,0,50) + (VectorRand() * 5));
			ent:Spawn();
			ent:Activate()
			ent:SetDamageOwner( dmgowner )
			ent:SetIsManhack( true )
			--ent:SetSpawner( placeholder )
			ent:SetThrower( dmgowner )
			--ent:SetNWEntity("Thrower", dmgowner)
			--ent:SetOrigin( dmgowner )
		end
...
local function ManhackPlayerDamage(target, dmg)	
	--[[
	if target:IsPlayer() then
		if dmginfo:GetInflictor():GetClass() == "npc_manhack" then
	...
	if inflictor:IsValid() and inflictor:IsNPC() and inflictor:GetClass() == "npc_manhack" and inflictor:GetIsManhack() then
		local ent = ents.Create( "weapon_ttt_manhack")
		dmg:SetAttacker( inflictor:GetThrower() )
...
ENT.Type   = "anim"
ENT.Base                = "base_anim"
ENT.PrintName           = "weapon_ttt_manhack"
...
scripted_ents.Register(ENT, "ttt_weapon_manhack", true)
]]
hook.Add("EntityTakeDamage","ManhackPlayerDamage",ManhackPlayerDamage)