...
local metaPly = FindMetaTable( "Player" )
 AccessorFunc( metaPly, "crowbared", "Crowbared", FORCE_BOOL )
SWEP.HoldType                = "melee"
...
local function CrowBarGround( player, inWater, onFloater, speed )
	local tname = Format("CrowBar_%d_%d", player:EntIndex(), math.ceil(CurTime()))
	timer.Create( tname, 1, 1, function() 
		player:SetCrowbared(false)
	end)
	
end

local function CrowBarDamage( target, dmg )
	if target:IsPlayer() and target:Alive() then
		if dmg:IsFallDamage() then
			--if IsValid( target:GetSuperPush() ) then
				if target:GetCrowbared() == true then
					local ent = ents.Create('weapon_zm_improvised')
...
hook.Add("EntityTakeDamage","CrowBarDamage", CrowBarDamage)
hook.Add("OnPlayerHitGround","CrowBarGround", CrowBarGround)