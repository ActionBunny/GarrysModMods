...
local function IdentifyBody(ply, rag)
   ...
   if not CORPSE.GetFound(rag, false) then
	  ...
	  if not IsValid(rag.real) then
		rag.real = true
	  end
	  
	  if deadply then
		if rag.real then
			deadply:SetNWBool("body_found", true) --hier abändern wenn fake + was passiert damit?
		else
		 ...
		 if traitor then
			...
			if rag.real then
				SendConfirmedTraitors(GetInnocentFilter(false)) --hier einhaken und fakerole erzeugen
			else
...
function CORPSE.Create(ply, attacker, dmginfo, real)
	if real then
		if not IsValid(ply) then return end
	   ...
	   hook.Run("TTTOnCorpseCreated", rag, ply) --schickt mit rag.real ob true or false aber wohin??
	   ...
	   rag.dmgtype = dmginfo:GetDamageType()
		...
		ply:SetNWString("fake_corps", rag.was_role)  --SetNWString fake_corps: wenn speiuler fake coprs hat dann rolle zuweisen
