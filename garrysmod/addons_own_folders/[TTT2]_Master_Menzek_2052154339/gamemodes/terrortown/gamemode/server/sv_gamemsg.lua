...
CreateConVar("ttt_block_generalchat", "false", {FCVAR_ARCHIVE, FCVAR_NOTIFY})
...
function GM:TTT2CanSeeChat(reader, sender, isTeam)
	-- if IsValid(sender) and sender:IsPlayer() and sender:Alive() and GetConVar("ttt_block_generalchat"):GetBool() and GetRoundState() == ROUND_ACTIVE then
		-- return false
	-- else
		return true
	-- end
...
function GM:PlayerSay(ply, text, teamOnly)
	...
	-- if GetRoundState() == ROUND_ACTIVE then
			-- if team_spec or not ply:Alive() then
		-- return text or ""
	-- end
		...
		-- elseif teamOnly and not team_spec and ply:IsSpecial() then
			...
			-- if ply:Alive() and GetConVar("ttt_block_generalchat"):GetBool() then
	...
	-- if ply:Team() ~= TEAM_INNOCENT then
		-- return ""
	-- end
