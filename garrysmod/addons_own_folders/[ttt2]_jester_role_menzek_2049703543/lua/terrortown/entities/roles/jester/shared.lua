...
CreateConVar("ttt_jester_winBonus", "6", {FCVAR_NOTIFY, FCVAR_ARCHIVE})
CreateConVar("ttt_jester_winBonusByDetective", "10", {FCVAR_NOTIFY, FCVAR_ARCHIVE})

function ROLE:PreInitialize()
	self.color = Color(245, 48, 155, 255)

	self.abbr = "jes" -- abbreviation
	self.visibleForTraitors = false -- other traitors can see this role / sync them with traitors
	...
	self.winBonus = GetConVar("ttt_jester_winBonus"):GetFloat()
...
hook.Add("TTTUlxDynamicRCVars", "TTTUlxDynamicJesCVars", function(tbl)
	...
	table.insert(tbl[ROLE_JESTER], {
		...
		choices = {
			...
			"1 - Jester wins if he's killed by any player, except traitors"--,
