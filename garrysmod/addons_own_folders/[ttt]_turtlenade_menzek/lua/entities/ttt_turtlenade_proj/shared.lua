...
AccessorFunc( metaEnts, "isturtle", "IsTurtle", FORCE_BOOL )
AccessorFunc( metaEnts, "turtlenade", "TurtleNade" )

CreateConVar( "ttt_turtlenade_number", "8", FCVAR_NOTIFY + FCVAR_ARCHIVE , "Adjust the amount of Turtles." )
CreateConVar( "ttt_turtlenade_innodamage", "10", FCVAR_NOTIFY + FCVAR_ARCHIVE , "Adjust the damage to innocents." )
CreateConVar( "ttt_turtlenade_traitordamage", "5", FCVAR_NOTIFY + FCVAR_ARCHIVE , "Adjust the damage to traitors." )

--local metaNPC = FindMetaTable("NPC")
--AccessorFunc( metaNPC, "origin", "Origin")
...
ENT.TurtleCount 	= GetConVar( "ttt_turtlenade_number" ):GetFloat()


TurtleNPCClass 		= "npc_headcrab_fast"
TurtleInnocentDamage  = GetConVar( "ttt_turtlenade_innodamage" ):GetFloat()
TurtleTraitorDamage   = GetConVar( "ttt_turtlenade_traitordamage" ):GetFloat()
...
function ENT:Explode(tr)
   ...
   if SERVER then
	 ...
	 ----placeholder = ents.Create( "ttt_turtlegrenade" )
	 ...
	 -- pull out of the surface
      ...
      self:SetDetonateExact(0)	
	     ...
	     for i=1,self.TurtleCount do
			...
			headturtle:SetIsTurtle( true )
			----headturtle:SetTurtleNade( placeholder )
			headturtle:SetNWEntity("Thrower", self:GetThrower())
			headturtle:SetName("ttt_turtlegrenade")
			--headturtle:SetNWEntity("Proj", self )
			--headturtle:SetName(self:GetThrower():GetName())
			headturtle:SetNoDraw(true)
			headturtle:SetHealth(1000)
			--print ( headturtle:GetNWEntity( "Proj" ) )
		end
...
function TurtleNadeDamage(victim, dmg)
	...
	if attacker:IsValid() and attacker:IsNPC() and attacker:GetClass() == TurtleNPCClass and attacker:GetIsTurtle() then
		local ent = ents.Create('weapon_ttt_turtlenade')
		if victim:IsTraitor() == false  then
			dmg:SetAttacker( attacker:GetNWEntity("Thrower") )
			----dmg:SetInflictor( attacker:GetTurtleNade() )
	...
	if victim:GetClass() == TurtleNPCClass and victim:GetIsTurtle() then
...
ENT.Type   = "anim"
ENT.Base                = "base_anim"
ENT.PrintName           = "ttt_turtlegrenade"
...
scripted_ents.Register(ENT, "ttt_turtlegrenade", true)
]]