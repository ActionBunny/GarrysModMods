if SERVER then
  ...
  CreateConVar( "ttt_staminup_speedscale_t", "1.75", FCVAR_NOTIFY + FCVAR_ARCHIVE + FCVAR_REPLICATED, "Adjust the speed a stamined traitor player has. 1 is default, 0 none." )
  CreateConVar( "ttt_staminup_speedscale_d", "1.35", FCVAR_NOTIFY + FCVAR_ARCHIVE + FCVAR_REPLICATED, "Adjust the speed a stamined detective player has. 1 is default, 0 none." )
  
staminModifierTraitor = GetConVar( "ttt_staminup_speedscale_t" ):GetFloat()
staminModifierDetective  = GetConVar( "ttt_staminup_speedscale_d" ):GetFloat() 
...
function SWEP:DrinkTheBottle()
  ...
  if !self.Owner:HasEquipmentItem(equip_id) then
    if CLIENT then
...
hook.Add("TTTPlayerSpeedModifier", "StaminUpSpeed", function(ply, _, _, refTbl)  
  local equip_id = TTT2 and "item_ttt_staminup" or EQUIP_STAMINUP
  if not IsValid(ply) or not ply:HasEquipmentItem(equip_id) or not ply:GetNWBool("StaminUpActive",false) then return end
	if (ply:GetTeam() == TEAM_TRAITOR) then
		refTbl[1] = refTbl[1] * staminModifierTraitor * (ply.speedrun_mul or 1)
	else	
		refTbl[1] = refTbl[1] * staminModifierDetective * (ply.speedrun_mul or 1)
	end
end)
