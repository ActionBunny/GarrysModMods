...
SWEP.Author			= "Hoff"
SWEP.Instructions	= "Damn straight. Dropable but cannot be picked up again. \nGrants short blastimmunity after Fall. \nOverall 50% blastdamagereduction."
...
local function RemoveFallDamage(target, dmginfo)
	if target:IsPlayer() and target.ShouldRemoveFallDamage then
		...
		target.endImmunityToBlast = target.endImmunityToBlast or 0
		--erzeugt explosion
		if dmginfo:IsFallDamage() then
			...
			if target:IsRole(ROLE_TRAITOR) then 
				target.endImmunityToBlast = CurTime() + 3
			else
				target.endImmunityToBlast = CurTime() + 1.2
			end
		end
		--negiert schaden	
		if dmginfo:IsFallDamage() then
		...
		if dmginfo:IsExplosionDamage() then
			if target.endImmunityToBlast > CurTime() then
				dmginfo:SetDamage(0)
			else
