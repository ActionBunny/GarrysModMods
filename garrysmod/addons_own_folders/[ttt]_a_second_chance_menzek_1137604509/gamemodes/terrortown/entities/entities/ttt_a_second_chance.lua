--CreateConVar( "ttt_asc_traitorscale", "200", FCVAR_NOTIFY + FCVAR_ARCHIVE , "Adjust scale of PercentageIncrease for Traitors. 200 is default, 100 is equal x1." )
--CreateConVar( "ttt_asc_detectivescale", "350", FCVAR_NOTIFY + FCVAR_ARCHIVE , "Adjust scale of PercentageIncrease for Detective. 350 is default, 100 is equal x1." )
...
if SERVER then
  ...
  resource.AddFile("vgui/ttt/icon_asc.vtf")
  resource.AddFile("vgui/ttt/perks/hud_asc.png")
...
local numASCPlayers = 0
local numASCTraitors = 0
local numASCAll = 0
hook.Add("TTTBeginRound", "TTTASCTTTBeginRound", function()
	numASCPlayers = 0
	numASCTraitors = 0
	...
	for _,v in pairs( player.GetHumans() ) do
		if not v:IsSpec() then
			if v:IsTraitor() then
				numASCTraitors = numASCTraitors + 1
			else
				numASCPlayers = numASCPlayers + 1
			end
		end
	end
	numASCAll = numASCPlayers + numASCTraitors 
end)
...
hook.Add("TTTOrderedEquipment", "TTTASC", function(ply, id, is_item)
  if id == EQUIP_ASC then
	...
	if ply:GetRole() == ROLE_TRAITOR then
		--ply.SecondChanceChance = math.Round( (200 / ( numASCAll ),0 )
		ply.SecondChanceChance = 25
	elseif ply:GetRole() == ROLE_DETECTIVE then
		--ply.SecondChanceChance = math.Round( 200 / ( numASCAll ),0 )
		ply.SecondChanceChance = 10
	end
  ...
  local plymeta = FindMetaTable( "Player" );
	AccessorFunc( plymeta, "ascchance", "ASCChance" )
  ...
  function SecondChance( victim, inflictor, attacker)
    ...
    if victim.shouldasc == true and SecondChanceRandom <= PlayerChance then
      ...
      timer.Create("TTTASC" .. victim:EntIndex() , 1 ,10, function()
		  ...
		  victim:SetASCChance( victim.SecondChanceChance )
		  if IsValid(victim) then
  ...
  if !IsValid(self) then return end
    ...
    body:Remove()
	self:SetHealth( self:GetASCChance() )
  ...
  function CheckifAsc(ply, attacker, dmg)
    if IsValid(attacker) and ply != attacker and attacker:IsPlayer() and attacker:HasEquipmentItem(EQUIP_ASC) then
	  ...
	  if ((attacker:GetRole() == ROLE_TRAITOR) and ( ply:GetRole() == ROLE_INNOCENT or ply:GetRole() == ROLE_DETECTIVE )) then
		attacker.SecondChanceChance = math.Round ( attacker.SecondChanceChance + ( ( 100/numASCPlayers ) * (1-(1/numASCAll) )), 0 )
	 elseif (attacker:GetRole() == ROLE_DETECTIVE and ply:GetRole() == ROLE_TRAITOR ) then
        attacker.SecondChanceChance = math.Round ( attacker.SecondChanceChance + ( ( 100/numASCPlayers ) * (1-(1/numASCAll) ) * 2.2 ), 0 )
      end
 		if attacker.SecondChanceChance > 99 then
			attacker.SecondChanceChance = 99
		end
	print( attacker.SecondChanceChance )
	  net.Start("ASCKill")
