---- Health dispenser
if SERVER then
util.AddNetworkString("HSMessage")
AddCSLuaFile()
end
if CLIENT then
   -- this entity can be DNA-sampled so we need some display info
   ENT.Icon = "vgui/ttt/icon_health"
   ENT.PrintName = "HealthStationM"
   ...
   ENT.TargetIDHint = {
	...
	net.Receive("HSMessage", function()
	  ...
	  if self:GetNWBool( "IsPoisoned", false ) then	
		...
		if ply.isPoisoned == false then
			...
			timer.Create(uni .. "poisonhealth", 1, 0, function()
				...
				local ent = ents.Create('ttt_health_station')
...
hook.Add("DoPlayerDeath", "DoPlayerDeathHealthstation", function( ply, attacker, dmg )
	if ply.isPoisoned then
		timer.Stop(ply:UniqueID() .. "poisonhealth" )
		ply.isPoisoned = false
	end
end)
hook.Add("TTTPrepareRound", "TTTPrepareRoundHealthStation", function()
