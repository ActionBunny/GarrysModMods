...
AccessorFunc( metaPly, "superpush", "SuperPush", FORCE_BOOL )
AccessorFunc( metaPly, "superthrower", "Superthrower" )
AccessorFunc( metaPly, "superdisc", "Superdisc" )
...
local function PushPullRadius(pos, pusher) --, placeholder)
   local radius = 650
   ...
   for k, target in pairs(ents.FindInSphere(pos, radius)) do
      if IsValid(target) then
         ...
         if target:IsPlayer() and (not target:IsFrozen()) and ((not target.was_pushed) or target.was_pushed.t != CurTime()) then
			...
			target:SetSuperPush( true )
			target:SetSuperthrower( pusher )
			--target:SetSuperdisc( placeholder )
...
function ENT:Explode(tr)
   if SERVER then
      ...
      self:SetSolid(SOLID_NONE)
		...
		--placeholder = ents.Create( "ttt_superdiscomb" )
      ...
      PushPullRadius(pos, self:GetThrower() )--, placeholder)
...
local function SuperDiscoGround( player, inWater, onFloater, speed )
	local tname = Format("SuperPush_%d_%d", player:EntIndex(), math.ceil(CurTime()))
	timer.Create( tname, 1, 1, function() 
		player:SetSuperPush(false)
	end)
	
end

local function SuperDiscoDamage( target, dmg )
	if target:IsPlayer() and target:Alive() then
		if dmg:IsFallDamage() then
			--if IsValid( target:GetSuperPush() ) then
				if target:GetSuperPush() == true then
					local ent = ents.Create('weapon_ttt_superdiscomb')
					dmg:SetAttacker( target:GetSuperthrower() )
...
hook.Add("EntityTakeDamage","SuperDiscoDamage", SuperDiscoDamage)
hook.Add("OnPlayerHitGround","SuperDiscoGround", SuperDiscoGround)
--[[
local ENT = {}
ENT.Type   = "anim"
ENT.Base                = "base_anim"
ENT.PrintName           = "weapon_ttt_superdiscomb"
...
scripted_ents.Register(ENT, "weapon_ttt_superdiscomb", true)
]]