...
ENT.Height = 1500 -- how far does it spring a player?
	...
	if IsValid(ply) then
		if ply:Alive() then
			if not ply:IsSpec() then
					...
					ply:SetSpringAttacker( self:GetOwner() )
					self:LiftPlayer(ply)						

					
					timer.Create( ply:UniqueID() .. "SpringMineDamage", 0.1, 1, function() 
						ply:TakeDamage( 40, self:GetDefOwn() , self )
						self:Remove()
					end)			
			end
		end
...
function ENT:LiftPlayer(ply)
	ply:SetVelocity(Vector(0, 0, self.Height))
	...
	function ENT:OnTakeDamage( dmginfo )
	  --################ if dmginfo:GetAttacker() == self:GetOwner() then return end
	   ...
	   if self:Health() < 0 then
		  ...
		  if IsValid(self:GetDefOwn()) then
			TraitorMsg(self:GetDefOwn(), "YOUR SPRINGMINE HAS BEEN DESTROYED!")
		  end
	   end
	end
		...
		if (IsValid(owner) then
			net.Start("TTT_SpringMineWarning")
				...
				if armed then
					net.WriteVector(self:GetPos())
					--net.WriteFloat(0)
					net.WriteString(self:GetPlacer():GetTeam())
				end
			net.Broadcast()
		end	
	end
