-- Lua die auf Server ausgeführt wird und serverseitig Stores erstellt
-- Abfrage der Stores vom Client.
...
local RoleEquipment = nil
local EquipNum = 15
local EquipNumDet = 10
local RandomRoleEquipment = {}

gameevent.Listen( "TTTBeginRound" )
hook.Add("TTTBeginRound", "TTTBeginRoundRandomShop", function()

	RandomRoleEquipment = {}
end
)

function GetEquipmentForRole(role) --immer abgefragt sobald c gedrückt wird
-- role: 1=T 2=D
   ...
   if not Equipment then
      ...
      Equipment = tbl
	  -- im Verlauf Equipment[role] nutzen
	end
												
											--[[	PrintTable(RandomRoleEquipment)
											if RandomRoleEquipment == {} then
												print( "{}")
											elseif RandomRoleEquipment == nil then
												print( "nil")
											elseif #RandomRoleEquipment == 0 then
												print( "0")
											else
												print( "x")
											end]]
	...
	if role == 1 then
		if #RandomRoleEquipment == 0 then
			RoleEquipment = Equipment[role]
			RoleEquipment = shuffle(RoleEquipment)
			RoleEquipment = shuffle(RoleEquipment)
			PrintTable( RoleEquipment )
			
			for i=1, EquipNum, 1 do
				RN = math.random( table.Count( RoleEquipment ) )
				table.insert(RandomRoleEquipment, RoleEquipment[RN])
				table.remove(RoleEquipment, RN)
				print( RN )
			end
		...
		return Equipment and RandomRoleEquipment or {}
	else
		if #RandomRoleEquipment == 0 then
			RoleEquipment = Equipment[role]
			RoleEquipment = shuffle(RoleEquipment)
