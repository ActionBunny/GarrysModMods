...
AccessorFunc(ENT, "incend", "Incend")
...
function StartFires(pos, tr, num, lifetime, explode, dmgowner, incend)
   for i=1, num do
	  ...
	  if IsValid(incend) then
         flame:SetIncend(incend)
      end
...
function ENT:Explode()
   ...
   util.BlastDamage(self:GetIncend(), dmgowner, pos, 300, 40)
end
...
function ENT:Think()
   ...
   if IsValid(self.firechild) then
      if self.next_hurt < CurTime() then
         ...
         dmg:SetInflictor(self:GetIncend())
   ...
   elseif self:GetVelocity() == Vector(0,0,0) then
      ...
      self.firechild = SpawnFire(self:GetPos(), self.fireparams.size, self.fireparams.growth, 999, self:GetDamageParent(), self:GetIncend())
