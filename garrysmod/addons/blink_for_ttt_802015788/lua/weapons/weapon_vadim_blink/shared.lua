...
local range                 = CreateConVar( "ttt_blink_range", "1768", cvf )
...
-- content loading
resource.AddSingleFile( "materials/vgui/ttt/vadim_blink.vtf" )
resource.AddSingleFile( "materials/vgui/ttt/vadim_blink.vmt" )

resource.AddSingleFile( "sound/vadim_blink/aim1.mp3" )
resource.AddSingleFile( "sound/vadim_blink/aim2.mp3" )
resource.AddSingleFile( "sound/vadim_blink/teleport1.mp3" )
resource.AddSingleFile( "sound/vadim_blink/teleport2.mp3" )
...
function SWEP:Think()
    ...
    elseif self.Active > 0 then
        ...
        timer.Create( label, 0.016, 0, function()
            if not IsValid( self ) then timer.Destroy( label ) return end
            if self.Charge < self.MaxCharge then
                self.Charge = math.min( self.MaxCharge, self.Charge + recharge:GetFloat() )
            else
                timer.Destroy( label )
            end
        end )
